...
In this project, I used data from Qualitative Activity Recognition of Weight Lifting Exercises, a barbell lifting study,, to predict different ways barbells are lifted.  The data are measurements from accelerometers on the belt, forearm, arm, and dumbell of 6 participants .  I applied machine learning algorithm to develop a model that would be able to predict one of the correct way and five of the incorrect ways to lift barbells.  The best prediction model, which has been created by the Gradient Boost learning algorithm, has an accuracy of 97.3% for in-sample data and an accuracy of 96.1% for out-of-sample data.  The model is able to achieve 100% accuracy on test case data 
...
I estimated the out of sample error using in-sample error. The model with the least in-sample error was Stochastic Gradient Boosting.  It had an in-sample accuracy of 97.3%  (2.7% error rate).  Next, I checked this estimate with cross-validation using test data. The error rate should not have increased significantly if there was no overfitting.
...
The Gradient Boosting prediction model accuracy on test data was 96.1% (3.9% error rate). The error rate estimate from The cross-validation was almost as low as in-sample estimate.  It was not likely that there was overfitting.
